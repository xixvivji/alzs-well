...
public class ConsentService {
    ...
    public ConsentService(JdbcClient jdbc,Clock clock,StaffAccessPolicyService staffAccess){this.jdbc=jdbc;this.clock=clock;this.staffAccess=staffAccess;}

    @Transactional public ConsentList active(String customerId,AuditActor actor){
        ensureCustomer(customerId);staffAccess.require(actor,customerId,"CONSENT_READ","CONSENT",null);OffsetDateTime now=OffsetDateTime.now(clock);
    ...
    @Transactional public Consent detailAudited(String customerId,UUID consentId,AuditActor actor){staffAccess.require(actor,customerId,"CONSENT_READ","CONSENT",consentId.toString());Consent value=find(customerId,consentId);audit(customerId,consentId,"CONSENT_READ",actor,null,hash("detail:"+consentId),"READ",OffsetDateTime.now(clock));return value;}

    @Transactional public Consent grant(String customerId,GrantCommand command,String idempotencyKey,AuditActor actor){
        ensureCustomer(customerId);staffAccess.require(actor,customerId,"CONSENT_WRITE","CONSENT",null);List<String> scopes=normalize(command.scopes());validateMatrix(command.purposeCode(),scopes);
    ...
    @Transactional public Consent withdraw(String customerId,UUID consentId,WithdrawCommand command,AuditActor actor){
        staffAccess.require(actor,customerId,"CONSENT_WRITE","CONSENT",consentId.toString());Consent before=find(customerId,consentId);OffsetDateTime now=OffsetDateTime.now(clock);
    ...
    @Transactional public ConsentHistory history(String customerId,UUID consentId,AuditActor actor){
        staffAccess.require(actor,customerId,"CONSENT_READ","CONSENT",consentId.toString());find(customerId,consentId);List<ConsentEvent> items=jdbc.sql("select * from customer_consent_event where consent_id=? order by occurred_at,event_id").param(consentId).query((rs,n)->new ConsentEvent(rs.getObject("event_id",UUID.class),rs.getString("event_type"),rs.getString("status_snapshot"),array(rs.getArray("scope_snapshot")),rs.getString("reason"),rs.getString("actor_id"),rs.getObject("occurred_at",OffsetDateTime.class),rs.getLong("row_version"))).list();
    ...
    @Transactional public DisclosureEvaluation evaluate(String customerId,DisclosureEvaluationCommand command,AuditActor actor){
        staffAccess.require(actor,customerId,"CONSENT_READ","CONSENT",command.consentId().toString());Consent consent=find(customerId,command.consentId());List<String> requested=normalize(command.requestedScopes());validateMatrix(command.purposeCode(),requested);OffsetDateTime now=OffsetDateTime.now(clock);
    ...
    private void contactEvent(UUID id,AuditActor actor,OffsetDateTime at,long version){jdbc.sql("insert into trusted_contact_event(event_id,contact_id,event_type,actor_id,reason,occurred_at,row_version,actor_principal_id,actor_customer_id,actor_session_id,actor_type,status_snapshot,scope_snapshot) select ?,t.contact_id,'REVOKED_BY_CONSENT',?,'CONSENT_WITHDRAWN',?,?,?,?,?,? ,t.status,coalesce((select array_agg(s.scope_code order by s.scope_code) from trusted_contact_scope s where s.contact_id=t.contact_id),'{}') from trusted_contact t where t.contact_id=?").params(UUID.randomUUID(),actor.legacyActorId(),at,version,actor.principalId(),actor.customerId(),actor.sessionId(),actor.actorType(),id).update();}
