...
public class ConsentService {
    ...
    @Transactional public Consent grant(String customerId,GrantCommand command,String idempotencyKey,AuditActor actor){
        ...
        Optional<Replay> replay=jdbc.sql("select consent_id,request_hash from customer_consent where customer_id=? and idempotency_key_hash=?").params(customerId,keyHash).query((rs,n)->new Replay(rs.getObject(1,UUID.class),rs.getString(2))).optional();
        if(replay.isPresent()){if(!secureEquals(replay.get().requestHash(),requestHash))throw new BusinessException(IDEMPOTENCY_CONFLICT);return find(customerId,replay.get().id());}
        OffsetDateTime now=OffsetDateTime.now(clock);UUID id=UUID.randomUUID();
        jdbc.sql("insert into customer_consent(consent_id,customer_id,purpose_code,status,granted_at,expires_at,row_version,created_at,updated_at,idempotency_key_hash,request_hash) values(?,?,?,'GRANTED',?,?,1,?,?,?,?)").params(id,customerId,command.purposeCode(),now,command.expiresAt(),now,now,keyHash,requestHash).update();
    ...
    private List<String> normalize(List<String> scopes){return scopes.stream().distinct().sorted().toList();} private List<String> array(Array value)throws java.sql.SQLException{return value==null?List.of():List.of((String[])value.getArray());}
