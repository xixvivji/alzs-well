...
public class ConsentService {
    ...
    @Transactional public ConsentHistory history(String customerId,UUID consentId,AuditActor actor){
        staffAccess.require(actor,customerId,"CUSTOMER_CONSENT_MANAGEMENT","CONSENT_READ","CONSENT",consentId.toString());find(customerId,consentId);List<ConsentEvent> items=jdbc.sql("select * from customer_consent_event where consent_id=? order by occurred_at,event_id").param(consentId).query((rs,n)->new ConsentEvent(rs.getObject("event_id",UUID.class),rs.getString("event_type"),rs.getString("status_snapshot"),array(rs.getArray("scope_snapshot")),rs.getString("reason"),rs.getString("actor_id"),rs.getObject("occurred_at",OffsetDateTime.class),rs.getLong("row_version"))).list();
    ...
    @Transactional public DisclosureEvaluation evaluate(String customerId,DisclosureEvaluationCommand command,AuditActor actor){
        staffAccess.require(actor,customerId,"CUSTOMER_CONSENT_MANAGEMENT","CONSENT_READ","CONSENT",command.consentId().toString());Consent consent=find(customerId,command.consentId());List<String> requested=normalize(command.requestedScopes());validateMatrix(command.purposeCode(),requested);OffsetDateTime now=OffsetDateTime.now(clock);
