...
public class ConsentController {
    ...
    public ResponseEntity<ApiResponse<ConsentList>> consents(@PathVariable @Pattern(regexp=CUSTOMER_ID_PATTERN) String customerId){
        return ApiResponses.ok("CONSENTS_RETRIEVED","유효한 목적별 동의를 조회했습니다.",service.active(customerId));
    }
    ...
    public ResponseEntity<ApiResponse<Consent>> consent(@PathVariable @Pattern(regexp=CUSTOMER_ID_PATTERN) String customerId,@PathVariable UUID consentId){
        return ApiResponses.ok("CONSENT_RETRIEVED","동의 상세를 조회했습니다.",service.detail(customerId,consentId));
    }
    ...
    public ResponseEntity<ApiResponse<Consent>> grant(@PathVariable @Pattern(regexp=CUSTOMER_ID_PATTERN) String customerId,
            @Valid @RequestBody GrantCommand command,Principal principal){
        return ApiResponses.created("CONSENT_GRANTED","목적별 동의를 등록했습니다.",service.grant(customerId,command,principal.getName()));
    }
    ...
    public ResponseEntity<ApiResponse<Consent>> withdraw(@PathVariable @Pattern(regexp=CUSTOMER_ID_PATTERN) String customerId,
            @PathVariable UUID consentId,@Valid @RequestBody WithdrawCommand command,Principal principal){
        return ApiResponses.ok("CONSENT_WITHDRAWN","동의를 철회했습니다.",service.withdraw(customerId,consentId,command,principal.getName()));
    }
    ...
    public ResponseEntity<ApiResponse<ConsentHistory>> history(@PathVariable @Pattern(regexp=CUSTOMER_ID_PATTERN) String customerId,@PathVariable UUID consentId){
        return ApiResponses.ok("CONSENT_HISTORY_RETRIEVED","동의 변경 이력을 조회했습니다.",service.history(customerId,consentId));
    }
    ...
    public ResponseEntity<ApiResponse<DisclosureEvaluation>> evaluate(@PathVariable @Pattern(regexp=CUSTOMER_ID_PATTERN) String customerId,
            @Valid @RequestBody DisclosureEvaluationCommand command){
        return ApiResponses.ok("DISCLOSURE_EVALUATED","최소정보 제공 가능성을 평가했습니다.",service.evaluate(customerId,command));
    }
}
