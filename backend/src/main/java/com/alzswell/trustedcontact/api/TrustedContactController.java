...
public class TrustedContactController {
    ...
    public ResponseEntity<ApiResponse<Contact>> update(@PathVariable @Pattern(regexp=CUSTOMER) String customerId,@PathVariable UUID contactId,@Valid @RequestBody UpdateCommand command,org.springframework.security.core.Authentication authentication){return ApiResponses.ok("TRUSTED_CONTACT_UPDATED","최소정보 범위를 변경했습니다.",service.update(customerId,contactId,command,AuditActor.from(authentication)));}
    ...
    public ResponseEntity<ApiResponse<Contact>> revoke(@PathVariable @Pattern(regexp=CUSTOMER) String customerId,@PathVariable UUID contactId,@Valid @RequestBody RevokeCommand command,org.springframework.security.core.Authentication authentication){return ApiResponses.ok("TRUSTED_CONTACT_REVOKED","신뢰연락인 지정을 철회했습니다.",service.revoke(customerId,contactId,command,AuditActor.from(authentication)));}
}
