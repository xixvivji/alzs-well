...
public class FinancialIntentService {
 ...
 public FinancialIntentService(JdbcClient jdbc,Clock clock,StaffAccessPolicyService staffAccess){this.jdbc=jdbc;this.clock=clock;this.staffAccess=staffAccess;}
 ...
 @Transactional public Intent create(String customer,Draft c,String key,AuditActor actor){
  String requestHash=hash(c.paymentContinuity()+"|"+c.explanationMode()+"|"+c.helpCondition()+"|"+String.join(",",c.shareScopes().stream().sorted().toList())); return command(customer,customer+":CREATE",key,requestHash,()->{
 ...
 @Transactional public Intent update(String customer,UUID id,Update c,String key,AuditActor actor){
  return command(customer,customer+":"+id+":UPDATE",key,hash(c.paymentContinuity()+"|"+c.explanationMode()+"|"+c.helpCondition()+"|"+String.join(",",c.shareScopes().stream().sorted().toList())+"|"+c.expectedVersion()),()->{
 ...
 @Transactional public Intent approve(String customer,UUID id,Approve c,String key,AuditActor actor){
  return command(customer,customer+":"+id+":APPROVE",key,hash("APPROVE|"+c.expectedVersion()),()->transition(customer,id,c.expectedVersion(),"DRAFT","APPROVED","APPROVED",null,actor));
 }
 @Transactional public Intent revoke(String customer,UUID id,Revoke c,String key,AuditActor actor){
  return command(customer,customer+":"+id+":REVOKE",key,hash("REVOKE|"+c.expectedVersion()+"|"+c.reason().trim()),()->transition(customer,id,c.expectedVersion(),"APPROVED","REVOKED","REVOKED",c.reason(),actor));
 }
 ...
 private Intent command(String customer,String scope,String key,String requestHash,Supplier<Intent> action){String keyHash=hash(key);int inserted=jdbc.sql("insert into financial_intent_command(command_scope,idempotency_key_hash,request_hash,created_at) values(?,?,?,?) on conflict do nothing").params(scope,keyHash,requestHash,OffsetDateTime.now(clock)).update();if(inserted==0){var row=jdbc.sql("select request_hash,result_intent_id,result_version from financial_intent_command where command_scope=? and idempotency_key_hash=?").params(scope,keyHash).query((r,n)->new Object[]{r.getString(1),r.getObject(2,UUID.class),r.getLong(3)}).single();if(!same(requestHash,(String)row[0]))throw new BusinessException(FinancialIntentErrorCode.IDEMPOTENCY_CONFLICT);return jdbc.sql("select f.customer_id,r.*,f.created_at,f.approved_at,f.revoked_at from financial_intent_revision r join financial_intent f using(intent_id) where f.customer_id=? and r.intent_id=? and r.version=?").params(customer,row[1],row[2]).query(this::mapRevision).optional().orElseThrow(()->new BusinessException(FinancialIntentErrorCode.NOT_FOUND));}Intent result=action.get();jdbc.sql("update financial_intent_command set result_intent_id=?,result_version=? where command_scope=? and idempotency_key_hash=?").params(result.intentId(),result.version(),scope,keyHash).update();return result;}
