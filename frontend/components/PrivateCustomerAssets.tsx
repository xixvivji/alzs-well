...
function DepositScreen({ assets, busy, amount, term, interest, setAmount, setTerm, simulate }: { assets: Assets; busy: string | null; amount: number; term: number; interest: InterestSimulation | null; setAmount: (value: number) => void; setTerm: (value: number) => void; simulate: () => void }) {
  ...
  const installmentSavings = product?.productType === "INSTALLMENT_SAVINGS";
  return <section className="asset-screen">
    ...
    <div className="product-two-column"><article className="panel asset-detail-card"><p className="label">대표 보유상품</p><h2>{holding?.displayName ?? "보유 예금 없음"}</h2><small>{holding?.institutionName} · {holding?.maskedAccountNumber}</small><dl><div><dt>현재 잔액</dt><dd>{money(holding?.currentBalance, holding?.currency)}</dd></div><div><dt>적용 금리</dt><dd>연 {holding?.annualInterestRate ?? "-"}%</dd></div><div><dt>만기일</dt><dd>{date(holding?.maturityDate)}</dd></div><div><dt>누적 이자</dt><dd>{money(holding?.accruedInterest, holding?.currency)}</dd></div></dl></article><article className="panel simulation-card"><p className="label">실행 없는 이자 계산</p><h2>{product?.productName ?? "예금상품"}</h2><p className="muted">{installmentSavings ? "매월 같은 금액을 적립하는 합성 적금의 예상 이자만 계산하며 실제 가입은 지원하지 않습니다." : (assets.depositProductDetail?.cautionText ?? product?.summary)}</p><div className="simulation-inputs"><label><span>{installmentSavings ? "월 납입액" : "예치금"}</span><input type="number" min={product?.minPrincipal ?? 10000} max={product?.maxPrincipal ?? 1000000000} step={10000} value={amount} onChange={(event) => setAmount(Number(event.target.value))} /></label><label><span>기간(개월)</span><input type="number" min={product?.minTermMonths ?? 1} max={product?.maxTermMonths ?? 120} value={term} onChange={(event) => setTerm(Number(event.target.value))} /></label></div><button className="primary-button" disabled={!product || busy !== null} onClick={simulate}>{busy === "deposit" ? "계산 중…" : "예상 이자 계산"}</button>{interest && <div className="simulation-result"><span>총 납입원금<strong>{money(interest.totalPrincipal, interest.currency)}</strong></span><span>세전 이자<strong>{money(interest.grossInterest, interest.currency)}</strong></span><span>예상 세금<strong>{money(interest.estimatedTax, interest.currency)}</strong></span><span>만기 예상액<strong>{money(interest.estimatedMaturityAmount, interest.currency)}</strong></span><small>가입·해지·외부 호출 없음</small></div>}</article></div>
