...
export function PrivateHelpHub() {
  ...
  return <div className="private-help-hub">
    ...
    <section className="help-member-analysis bank-panel"><header><div><p>로그인 회원 장기 변화</p><h3>평소와 최근을 설명 가능한 값으로 비교</h3></div><Link href="/banking/safety">전체 근거·본인 확인 →</Link></header><div>{overview.baselines.slice(0, 3).map((item) => <article key={item.baselineId}><span>{featureLabel(item.featureCode)}</span><strong>{item.baselineValue}{unitLabel(item.unit)} → {item.currentValue}{unitLabel(item.unit)}</strong><small>{item.comparisonText || "현재 기준 범위 안에서 관찰 중입니다."}</small></article>)}</div>{!overview.baselines.length && <p className="help-analysis-empty">아직 계산된 회원 기준선이 없습니다. 거래 관찰 기간이 쌓이면 최근 30일과 이전 구간을 비교합니다.</p>}<footer>회원 계좌·거래와 같은 고객 ID의 기준선만 사용 · 진단·사기 판정·자동 금융조치 없음</footer></section>
