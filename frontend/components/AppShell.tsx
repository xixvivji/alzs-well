...
export function AppShell({ mode, title, staffRole = "shared", children }: { mode: "customer" | "staff"; title: string; staffRole?: "protection" | "admin" | "shared"; children: ReactNode }) {
  ...
  return <div className={`app-shell ${mode}`}>
    ...
    <aside>
      ...
      <nav className="side-nav" aria-label={navigationLabel}>{links.map(([href, label, step]) => <Link aria-current={isActive(href) ? "page" : undefined} className={isActive(href) ? "active" : ""} href={href} key={href}><span aria-hidden="true">{step}</span><strong>{label}</strong></Link>)}</nav>
    ...
    <main className="app-content" id="app-main" tabIndex={-1}>
      <div className="mobile-app-bar">
        ...
        <details className="mobile-navigation"><summary>전체 메뉴</summary><nav aria-label={`모바일 ${navigationLabel}`}>
          {links.map(([href, label, step]) => <Link aria-current={isActive(href) ? "page" : undefined} className={isActive(href) ? "active" : ""} href={href} key={href}><span aria-hidden="true">{step}</span>{label}</Link>)}
          {mode === "staff" && <Link className="mobile-channel-link" href="/demo">고객 안내 화면으로</Link>}
          <Link href="/">금융서비스 홈으로</Link>
        </nav></details>
      </div>
      <header>
        <div><p className="app-kicker">{mode === "customer" ? "금융생활 도움 서비스" : staffRole === "admin" ? "관리·준법 통제 채널" : "보호업무 운영 채널"}</p><h1>{title}</h1><p className="app-subtitle">{mode === "customer" ? "평소와 달라진 금융생활을 쉬운 말로 확인합니다." : staffRole === "admin" ? "정책·감사·AI 안전 경계를 관리 역할로 확인합니다." : "고객 응답과 승인된 근거를 바탕으로 사람이 최종 결정합니다."}</p></div>
        {mode === "customer" ? <div className="customer-header-actions"><MemberSessionStatus /></div> : <div className="customer-header-actions"><MemberSessionStatus /><div className="service-status"><i /><span><small>안전 운영</small><strong>검증 데이터 연결</strong></span></div></div>}
      </header>
