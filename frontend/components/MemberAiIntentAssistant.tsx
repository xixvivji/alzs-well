...
import type { IntentSuggestion } from "../lib/ai-financial-assistance";
...
export function MemberAiIntentAssistant({ session, initialIntent, onIntentChange }: { session: PrivateCustomerSession; initialIntent: FinancialIntent | null; onIntentChange?: (intent: FinancialIntent) => void }) {
  ...
  return <section className="member-ai-intent bank-panel">
    <header><div><h3>AI 금융생활 의향서</h3><p>편한 말로 입력하고, 직접 확인해 저장하세요.</p></div><span>회원 로그인 연결</span></header>
    ...
    <button className="secondary-button" disabled={busy !== "" || utterance.trim().length < 4} onClick={() => void run("suggest", async () => { const result = await suggestPrivateFinancialIntent(utterance); setSuggestion(result); setPaymentContinuity(result.suggestion.paymentContinuity); setExplanationMode(result.suggestion.explanationMode); setHelpCondition(result.suggestion.helpCondition); setSummary(result.summary); setNotice(result.fallbackUsed ? "검증된 안전 템플릿으로 초안을 정리했습니다." : "AI가 문장을 구조화했습니다. 아래 항목을 확인해 주세요."); })}>{busy === "suggest" ? "AI가 정리 중…" : "AI로 초안 정리"}</button>
    {summary && <p className="ai-summary">{summary}</p>}
    {suggestion?.needsClarification && <section className="member-ai-questions" aria-labelledby="intent-questions-title"><strong id="intent-questions-title">저장 전에 확인해 주세요</strong>{suggestion.clarifyingQuestions.map((question) => <p key={question}>• {question}</p>)}</section>}
    {suggestion && <details className="member-ai-evidence"><summary>AI가 문장에서 확인한 근거</summary>{suggestion.evidence.map((item) => <p key={item.field}><span>{fieldLabel(item.field)}</span><q>{item.excerpt}</q><small>근거 일치도 {Math.round(item.confidence * 100)}%</small></p>)}</details>}
    ...
    <div className="member-ai-actions">{!intent || intent.status === "REVOKED" ? <button disabled={busy !== ""} onClick={() => void run("save", async () => { const saved = await createIntent(session, { paymentContinuity, explanationMode, helpCondition, shareScopes: [] }); setIntent(saved); onIntentChange?.(saved); setNotice("로그인 회원의 수정 가능한 초안으로 저장했습니다."); })}>{busy === "save" ? "저장 중…" : "회원 의향서 초안 저장"}</button> : intent.status === "DRAFT" ? <button disabled={busy !== ""} onClick={() => void run("approve", async () => { const approved = await approveIntent(session, intent); setIntent(approved); onIntentChange?.(approved); setNotice("법적 효력 제한을 확인하고 본인이 승인했습니다."); })}>{busy === "approve" ? "승인 중…" : "확인 후 본인 승인"}</button> : <span className="status-pill safe">승인 완료 · v{intent.version}</span>}</div>
...
function fieldLabel(field: string) {
  return ({ paymentContinuity: "필수 납부", explanationMode: "설명 방식", helpCondition: "도움 조건", shareScopes: "행원 공유" } as Record<string, string>)[field] ?? field;
}
