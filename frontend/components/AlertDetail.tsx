...
export function AlertDetail({ alertId }: { alertId: string }) {
  ...
  async function answer(branch: AnswerBranch) {
    ...
    if (!active) return;
    if (branch === "later") {
      setResult({ message: "지금 결정하지 않아도 괜찮습니다. 알림을 그대로 남겨두었습니다.", currentState: "확인 대기 유지" });
      return;
    }
