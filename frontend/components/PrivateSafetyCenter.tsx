...
export function PrivateSafetyCenter() {
  ...
  if (!session && busy) return <section className="bank-panel banking-loading" aria-live="polite" aria-busy="true"><span className="bank-spinner loading loading-spinner loading-lg" aria-hidden="true" /><p>금융생활 변화를 확인하고 있습니다.</p></section>;
  ...
  return <div className="safety-center">
    ...
    <section className="bank-panel safety-alert-panel" id="context-check" aria-labelledby="context-check-title">
      <header>
        <div><p>내가 직접 확인</p><h3 id="context-check-title">금융활동 맥락 확인</h3></div>
        {bundle.alerts.length > 0 && <label className="alert-selector"><span>확인할 변화</span><select value={selectedAlertId} disabled={busy} onChange={(event) => void chooseAlert(event.target.value)}>{bundle.alerts.map((item) => <option value={item.alertId} key={item.alertId}>{feature(item.reasonCode)} · {state(item.state)}</option>)}</select></label>}
      </header>
      {bundle.selectedAlert ? <>
        <div className="context-question"><span aria-hidden="true">?</span><div><strong>{bundle.contextQuestion}</strong><small>이 선택은 상황을 이해하기 위한 확인이며, 치매 진단이나 금융 불이익으로 이어지지 않습니다.</small></div></div>
        <p className="context-consequence">선택 내용은 현재 상태와 확인 이력에만 반영됩니다. 송금·지급정지·상품 가입·가족 연락은 자동으로 실행되지 않습니다.</p>
        <div className="context-options" aria-label="금융활동 맥락 선택">{bundle.contextOptions.map((option) => <button className="btn btn-outline" disabled={busy || !["AWAITING_CONTEXT","DEFERRED"].includes(bundle.selectedAlert!.state)} onClick={() => void respond(option.responseCode)} key={option.responseCode}><strong>{option.label}</strong><small>{option.description}</small></button>)}</div>
        <details className="evidence-disclosure">
          <summary>선택·처리 이력 자세히 보기 <span>{bundle.audit.length}건</span></summary>
          <div className="audit-timeline">{bundle.audit.map((item) => <article key={item.auditEventId}><i /><time>{dateTime(item.createdAt)}</time><p><strong>{event(item.eventType)}</strong><small>{state(item.previousState)} → {state(item.resultingState)}</small></p><code>{item.integrityHash.slice(0,10)}…</code></article>)}</div>
        </details>
      </> : <Empty text="확인이 필요한 알림이 없습니다. 주의·오탐 전체 시연은 안내형 도움받기에서 체험할 수 있습니다." />}
    </section>
