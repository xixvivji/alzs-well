...
export function PrivateSafetyCenter() {
  ...
  const refresh = useCallback(async (active: PrivateCustomerSession, alertId?: string) => {
    const result = await loadSafetyCenter(active, alertId); setBundle(result); setSelectedAlertId(result.selectedAlert?.alertId ?? "");
  }, []);
  useEffect(() => { void restorePrivateCustomerSession().then(async (active) => { setSession(active); await refresh(active); }).catch((reason) => setError(message(reason))).finally(() => setBusy(false)); }, [refresh]);
  async function chooseAlert(alertId: string) { if (!session) return; setSelectedAlertId(alertId); setBusy(true); setError(""); try { await refresh(session, alertId); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
  async function respond(code: string) { if (!session || !bundle?.selectedAlert) return; setBusy(true); setError(""); setNotice(""); try { await respondToSafetyAlert(session, bundle.selectedAlert, code); await refresh(session, bundle.selectedAlert.alertId); setNotice("선택한 생활맥락을 반영했습니다. 자동 금융 조치 없이 상태와 감사이력만 갱신했습니다."); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
