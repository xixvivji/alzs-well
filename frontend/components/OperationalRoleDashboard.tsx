...
export function OperationalRoleDashboard({ mode }: { mode: "staff" | "admin" }) {
  ...
  if (!session && !error) return <section className="panel operational-role-gate" aria-busy="true" aria-live="polite"><p>로그인 권한을 확인하고 있습니다.</p></section>;
  if (!session) return <section className="panel operational-role-gate"><div><p className="label">LOGIN REQUIRED</p><h2>운영자 로그인이 필요합니다.</h2><p>{error || "운영자 계정으로 로그인해 주세요."}</p></div><Link href="/staff/login">운영 채널 로그인</Link></section>;
  ...
  return <section className="operational-role-dashboard"><header><div><h2>{accountDisplayName(session.displayName)} · 관리 현황</h2></div></header><div>{cards.map(([label,count]) => <article key={String(label)}><span>{label}</span><strong>{count}</strong></article>)}</div>
