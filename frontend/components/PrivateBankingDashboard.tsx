...
export function PrivateBankingDashboard() {
  ...
  const firstTask = changeIndicator
    ? {
        ...
        secondaryHref: "/banking/help#help-analysis",
      }
    : changeStatusLoading
      ? {
          label: "금융생활 변화 확인 중",
  ...
  return <div className="private-banking-dashboard">
    ...
    <section className="banking-welcome"><div><p>{accountDisplayName(session.displayName)}님, 안녕하세요.</p><h2>오늘도 편안한 금융생활 되세요.</h2><span>{overview.summary.dataAsOf} 기준</span></div><div><small>총 자산</small><strong>{won(overview.summary.totalAssets)}</strong><span>순자산 {won(overview.summary.netAssets)}</span></div></section>
