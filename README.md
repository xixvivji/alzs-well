...
- Next.js 금융 포털은 고객·행원·관리자 채널을 분리하고, 구현 237개·계획 23개·외부 참고 22개 operation을 생성 카탈로그로 검증한다. 고객 안심 보호센터는 알림·AI 의향서·장기 변화·감사이력을 한 화면에 묶는다. 공개 금융서비스는 회원가입 없이 `demo001`~`demo300` 합성 회원만 로그인하며, 회원별 계좌·거래·카드·예금·대출·투자 데이터를 분리한다. Bearer token은 Vercel Secure·HttpOnly 쿠키로만 관리하고 고객 API는 customerId 소유권을 다시 검증한다.
