...
2026-09-06 개발 추가: 로그인 행원 사건의 선택형 Bedrock 검토 초안 경로를 구현했다. 기본 비활성화이며 실제 모델 호출·배포 검증은 미완료다. 고객 흐름·Arctic-ko 검색은 유지한다. [생성형 초안 범위와 활성화 조건](docs/BEDROCK_STAFF_DRAFT.md). 아래 규모는 기존 문서 점검 시점 기준이며 이 변경의 카탈로그는 문서 284개·구현 240개다.
