...
2026-09-05 문서 점검: 아래 버전별 보고서는 당시 모델·데이터·승인 범위의 기록이다. 이번 문서 수정에서 검색 평가를 새로 실행하지 않았다.

- 합성 CI 회귀: 14개 chunk·17개 질의.
- 공식 근거 골든셋: 27건(답변형 21건·무응답형 6건). 승인·승격 기록은 `arctic-ko-promotion-decision-v3.md`.
- 추가 검수 후보: 150건(답변형 105건·무응답형 45건). 일부 검수·순위 보완 이력은 `independent-review-near-match-human-v1.md`, `independent-review-temporal-filter-v1.md`, `independent-review-intent-ranking-v1.md`와 해당 검수 파일을 따른다.
- 의도 순위 v4 보고서는 후보 답변형 Top-1 90/105·Top-3 105/105와 별도 공식 답변형 Top-1 18/21·Top-3 21/21을 기록한다. 이 보고서 수치를 이후 다른 relevance 버전의 측정값과 혼합하지 않는다.

원본 후보의 `PENDING`은 후속 개별 승인 기록과 별도로 유지되는 작성 시점 상태다. 일부 승인이나 내부 검수를 전체 150건의 독립 외부 검수 완료로 해석하지 않는다.
