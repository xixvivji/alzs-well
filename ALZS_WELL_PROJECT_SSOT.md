...
> 버전: **1.5**
> 구현 점검 기준일: **2026-09-05** (코드 `517781c`, 배포 상태는 별도 기록)
> 참가 대회: [2026 금융 AI Challenge](https://daker.ai/public/hackathons/2026-finance-ai-challenge)  
> 제출 마감: **2026-09-07 10:00 KST**

이 문서는 제품의 목적·안전 경계와 현행 구현 범위를 정리한다. 구현 완료 여부는 코드·테스트, 실제 배포 여부는 배포 증적으로 확인한다. 차이가 발견되면 이 문서를 갱신하며, 문서의 계획만으로 구현·배포 완료를 주장하지 않는다. 과거 방향은 Git 이력에서 확인한다.

## 0. 현행 구현과 검증 상태

- 고객·행원·관리자 Next.js 포털은 `app/**/page.tsx` 기준 26개 라우트다. 핵심 시연은 고객 변화 확인 → 행원 검토 → 안내계획 승인으로 구성하며, 회원 경로 `/banking/**`와 익명 시나리오 `/demo/**`는 별도 인증 범위를 사용한다.
- Spring Boot 3.5.16·Java 21이 인증·권한·정책·상태전이·감사를 담당하고, 내부 FastAPI가 검색·구조화 지원·EWMA/CUSUM 분석을 담당한다. 고객·직원 프런트는 Vercel, 업무·AI는 AWS EC2 각 1대, DB는 Private RDS 구성이다.
- API 명세 283개 중 업무 구현 238개와 별도 직원 bootstrap 1개를 포함해 코드 operation은 239개다. 정적 프런트 호출 집계는 158/239이며 BFF·동적 호출은 별도 해석한다. 모든 API가 고객 화면 기능인 것은 아니다.
- `synthetic-v3.1.0`의 PUBLIC 생성 규격은 고객 300명·계좌 600개·회원당 거래 720건(총 216,000건)이다. V77과 함께 코드에 반영됐으며 AWS 재배포·새 fixture 적재·세 역할 E2E는 보류 상태다.
- 검색 기본값은 Hash이며, 승인된 AWS staging 구성은 `local-arctic-ko` 1024차원·`STAGED_APPROVED`를 사용한다. Arctic-ko는 임베딩 모델이다. 쉬운 설명·의향 초안·행원 안내에는 구조화 규칙과 템플릿을 사용하며 생성형 LLM 연결은 후속 선택 기능이다.
- 과거 검증 수치는 각 평가 보고서의 모델·데이터셋·시점에 한정한다. 문서 갱신은 재평가나 운영 재검증을 의미하지 않는다.
- §18의 일정과 §19의 항목은 초기 계획 및 출시 재검증 요구사항이다. 미체크 항목을 곧바로 기능 미구현으로 해석하지 않으며, 현행 상태는 [문서 점검 기준](docs/CURRENT_IMPLEMENTATION_STATUS.md)을 따른다.
...
> **ALZ's well은 치매를 진단하는 AI가 아니라, 고객이 금융생활 의사와 도움받을 범위를 미리 준비하고, 정기납부 누락·중복거래·업무 미완료·반복확인 같은 개인별 금융관리 행동변화가 생기면 본인에게 먼저 확인한 뒤, 필요한 경우 행원이 고객의 의사에 맞는 보호서비스를 안내하도록 돕는 B2B2C 금융생활 연속성 플랫폼이다. 현재 MVP는 규칙·템플릿과 내부 FastAPI의 Arctic-ko 검색·EWMA/CUSUM 분석을 사용한다. 생성형 LLM과 실제 은행 내부문서 도입은 후속 목표다.**
...
| RAG | 승인된 공식·합성 자료에 대한 권한형 검색 구현, staging Arctic-ko 사용. 실제 은행 내부 규정 도입은 PoC 범위 |
...
현재 MVP는 **고객·행원·관리자 26개 페이지 라우트, 회원별 합성 금융데이터, 금융생활 의향·변화 확인·보호업무 흐름**을 제공한다. 아래 세 화면은 핵심 시연 단계이며 전체 구현 화면 수가 아니다. 익명 시나리오는 고정된 맥락 비교를, 로그인 회원 화면은 해당 회원의 저장 데이터를 사용한다.
...
- 현행 PUBLIC 생성 규격: **300개 합성 회원, 회원당 720건·총 216,000건 거래** (`synthetic-v3.1.0`, AWS 적재는 별도 확인)
- 정상 패턴 80개
- 설명 가능한 생활변화 80개
- 행원 검토 필요 시나리오 80개
...
기반모델을 처음부터 만들지 않는다. 소규모 팀과 공모전 일정에서 데이터, GPU, 평가, 보안을 함께 관리하며 승인된 임베딩 모델과 설명 가능한 분석을 사용한다.

현재는 내부 FastAPI의 Arctic-ko 검색·EWMA/CUSUM 분석과 검증된 구조화 규칙·템플릿을 사용한다. Hash는 기본 임베딩 설정이며 AWS staging의 승인 모델 설정과 구분한다. 자체 호스팅 생성형 LLM과 실제 은행 내부문서 도입은 후속 PoC 목표다. 이 절의 P0/P1 표기는 초기 개발 우선순위이며 현행 구현 여부는 §0을 따른다.
...
| 권한형 벡터 RAG | 승인된 공식·합성 자료 검색 구현, 실제 은행 내부 규정 적재는 후속 | 현행 구현 / 실제 내부문서 PoC |
...
업무 서버는 **Spring Boot 기반 모듈형 모놀리스**이며 내부 Python FastAPI를 별도 AI EC2에서 실행한다. GPU 및 생성형 LLM 서버는 현재 필수 구성에 포함하지 않는다.
...
- PostgreSQL, Flyway
- Next.js 16·React 19 기반 프론트엔드와 HttpOnly 쿠키 BFF·JSON REST API 계약
- Spring AI는 선택 사용
- Docker 기반 배포
...
```mermaid
flowchart LR
    A["Next.js 고객·행원·관리자 / Vercel BFF"] --> B["Spring Boot JSON REST API"]
    B --> C["ledger"]
    B --> D["detection"]
    ...
    D --> I
    E --> I
    F --> J["내부 FastAPI · Arctic-ko 검색 · EWMA/CUSUM · 템플릿"]
    K --> J
    G --> I
    H --> I
```
...
| `demo` | 익명 세션 capability, 시나리오 seed, `demoRunId`, Reset, 핵심 3단계 시연 |
...
- 공개 금융서비스 체험은 회원가입 없이 성공한 `PUBLIC` 합성 fixture의 `demo001`~`demo300`만 고객으로 허용한다. 각 인증 주체는 서로 다른 합성 고객·계좌·거래·정기납부·수취인·카드·예금·대출·투자·외환·연금·신탁 snapshot에 연결하며 모든 고객 API가 URL의 customerId와 인증 소유권을 다시 비교한다. 공개 합성 운영 채널은 `staff001`~`staff005`의 `PROTECTION_STAFF`, `admin001`~`admin002`의 `DETECTION_ADMIN` 역할만 허용하고 고객 역할과 상호 교차 호출을 거부한다. access·refresh token 원문은 Vercel BFF가 Secure·HttpOnly·SameSite=Strict 쿠키로만 보관하고 브라우저 JavaScript·웹 저장소·응답 본문에는 노출하지 않는다. 기존 공용 `synthetic-customer`는 공개 인증 경로에서 거부한다.
...
현재 AWS는 업무 EC2와 AI EC2를 분리하고 GPU 없이 실행한다. 생성형 sLLM 추가 시 자원 요구량과 비용을 별도 검증한다.
...
아래는 인원 수가 아닌 책임 영역이다. 소규모 팀에서 겸임할 수 있으며 실제 담당자·독립 검토 여부는 제출 시 확인한다.
...
아래는 초기 일정이다. 현재 배포·미완료 작업 상태는 §0과 배포 체크리스트를 따른다.
...
체크박스는 해당 릴리스의 재검증 승인 여부다. 구현 여부와 동일하지 않다. 2026-09-05 코드 기준 재배포·E2E·최종 릴리스는 보류 중이며 이전 운영 검증은 날짜별 기록으로 보존한다.

### 기능 완료

- [ ] 고객 확인·행원 검토·안내 승인 세 단계로 핵심 가치가 설명된다.
...
| 단계 | 범위 | 검증 목표 |
|---|---|---|
| 1. 공모전 MVP | 합성 회원 포털·의향서·보호서비스 안내, 고객·행원·관리자 화면, 정상·주의·오탐 시연 | 사전준비·변화확인·업무연결의 작동성·설명가능성·안전게이트 |
...
> **공모전 MVP에서는 규칙·통계 엔진과 검증된 템플릿·공식 공개근거 카탈로그가 합성 의향서·금융관리 변화·보호서비스·업무초안을 연결하고, 현재 내부 FastAPI의 임베딩 검색·통계 분석이 근거 탐색과 변화 설명을 지원하며, 최종 선택·판단·실행은 고객과 행원이 담당합니다.**
...
> 인지기능이 저하되면 돈이 사라지는 것만이 문제가 아닙니다. 납부와 자산관리가 끊기고, 본인이 원했던 금융생활 방식도 전달되지 않을 수 있습니다. ALZ's well은 건강할 때 금융생활 의사와 도움받을 범위를 미리 준비하고, 평소와 다른 변화가 생기면 질병으로 단정하지 않고 본인에게 먼저 확인합니다. 공모전 MVP는 합성 의향서와 결정론적 규칙·템플릿·공식 공개근거로 보호서비스와 행원 업무초안을 연결합니다. Arctic-ko 검색과 설명 가능한 통계 분석을 사용하며 생성형 LLM과 실제 은행 내부문서는 후속 목표입니다. 최종 선택과 판단은 고객과 행원이 합니다.
