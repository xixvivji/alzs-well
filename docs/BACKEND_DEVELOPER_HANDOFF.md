...
- 기존 Flyway V1~V63은 수정하지 않는다. 다음 스키마 변경은 반드시 V64부터 추가한다.
...
- V62는 합성 환율, 마스킹 외화계좌, 합성 해외송금 이력과 실행 없는 환전 모의계산을 추가한다. V63은 검증된 AI ingestion import와 `chunkId ↔ passageId` 추가 전용 binding을 추가한다. 실제 환전·송금·외부 호출은 실행하지 않는다. AI 키워드 검색은 V61이다.
