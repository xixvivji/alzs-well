...
- API 카탈로그는 264개이며 구현된 업무 API는 123개다. 직원 capability 발급 경로까지 포함한 전체 코드 operation은 124개다.
- 나머지 141개는 구현 완료가 아니라 P1·P2·참조 카탈로그다.
- 공개 데모는 완전 합성데이터만 사용한다. 실제 금융기관, 마이데이터, 가족 연락, 송금, 주문, 차단, 외부 LLM을 호출하지 않는다.
- 기존 Flyway V1~V36은 수정하지 않는다. 다음 스키마 변경은 반드시 V37부터 추가한다.
