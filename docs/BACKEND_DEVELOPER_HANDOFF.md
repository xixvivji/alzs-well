...
- API 카탈로그는 282개이며 문서화된 업무 API는 237개다. 직원 capability 발급 경로까지 포함한 전체 코드 operation은 238개다.
- 나머지 문서 operation 45개 중 23개는 계획, 22개는 참조 전용이다. 코드 전용 직원 capability 1개는 282개 업무 카탈로그에 포함하지 않는다.
...
2. 구현된 237개 문서 operation의 도메인별 summary·개별 오류 예시를 지속적으로 구체화
