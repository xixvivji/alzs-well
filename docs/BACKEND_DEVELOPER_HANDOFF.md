...
- API 카탈로그는 271개이며 구현된 업무 API는 179개다. 직원 capability 발급 경로까지 포함한 전체 코드 operation은 180개다.
- 나머지 92개는 구현 완료가 아니라 P1·P2·참조 카탈로그다. 코드 전용 직원 capability 1개는 271개 업무 카탈로그에 포함하지 않는다.
- 공개 데모는 완전 합성데이터만 사용한다. 실제 금융기관, 마이데이터, 가족 연락, 송금, 주문, 차단, 외부 LLM을 호출하지 않는다.
- 기존 Flyway V1~V46은 수정하지 않는다. 다음 스키마 변경은 반드시 V47부터 추가한다.
...
미구현 92개를 한꺼번에 Controller로 생성하지 않는다. 다음 wave에서 필요한 5~10개를 골라 상세 계약을 `CONTRACT`로 확정한 뒤 구현한다. 금융 실행 API와 신뢰연락인 실제 연락 API는 계속 `REFERENCE_ONLY` 또는 외부 소유다.

## 12. 추천 다음 작업

1. capability의 `sessionStorage` 불일치 해소
2. 구현된 180개 operation의 도메인별 summary·개별 오류 예시를 지속적으로 구체화
