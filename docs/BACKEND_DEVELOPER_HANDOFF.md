...
> 점검 기준: 2026-09-05 `517781c` (develop 반영 대기)
...
- API 카탈로그는 283개이며 문서화된 업무 API는 238개다. 직원 capability 발급 경로까지 포함한 전체 코드 operation은 239개다.
- 나머지 문서 operation 45개 중 23개는 계획, 22개는 참조 전용이다. 코드 전용 직원 capability 1개는 283개 업무 카탈로그에 포함하지 않는다.
- 공개 데모는 완전 합성데이터만 사용한다. 실제 금융기관, 마이데이터, 가족 연락, 송금, 주문, 차단, 외부 LLM을 호출하지 않는다.
- 기존 Flyway V1~V77은 수정하지 않는다. 다음 스키마 변경은 반드시 V78부터 추가한다. V77은 PUBLIC v3.1의 생활거래 다양화를 지원하며 새 코드·fixture의 AWS 적용은 보류 중이다.
...
- V76은 고객에게 `CUSTOMER` audience로 승인된 지식 문서 조회·검색 권한을 부여한다. 문서 ACL·효력기간·감사 검증은 그대로 유지한다.
...
2. 구현된 238개 문서 operation의 도메인별 summary·개별 오류 예시를 지속적으로 구체화
