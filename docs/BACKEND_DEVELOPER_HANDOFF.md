...
- API 카탈로그는 261개이며 구현된 업무 API는 79개다. 직원 capability 발급 경로까지 포함한 코드 operation은 80개다.
- 나머지 182개는 구현 완료가 아니라 P1·P2·참조 카탈로그다.
- 공개 데모는 완전 합성데이터만 사용한다. 실제 금융기관, 마이데이터, 가족 연락, 송금, 주문, 차단, 외부 LLM을 호출하지 않는다.
- 기존 Flyway V1~V25는 수정하지 않는다. 다음 스키마 변경은 반드시 V26부터 추가한다.
...
2. 구현된 80개 operation에 OpenAPI 설명·예시·authority metadata 보강
3. 운영형 사건 후속 일정 API 계약 확정
