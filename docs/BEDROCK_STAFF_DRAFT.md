...
- 운영 UI에서 일반 지침의 중복 송금 항목을 반복 확인 사건에 섞는 출력도 확인했다. 사건에 없는 세 가지 MVP 신호 표현을 포함하면 전체 초안을 폐기하는 검증과 회귀 테스트를 추가했다. 이 어휘 기반 검사는 모든 사실 오류를 탐지하는 의미 검증기가 아니며 사람의 원문 확인을 대체하지 않는다.
