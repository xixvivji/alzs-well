...
curl --fail --silent --show-error "${BASE_URL}/api/v1/system/versions" \
  ...
  | jq -e '.code == "SYSTEM_VERSIONS_RETRIEVED" and .data.schemaVersion == "30"' > /dev/null
...
echo "Compose smoke test passed: PostgreSQL, Flyway V30, backend, gateway, readiness, guardrails, demo session"
